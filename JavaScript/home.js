...
window.addEventListener("resize", () => {
    ...
    currentPage = 0;
    // Re-create dots for new screen size
    createDots();
...
// Dynamic dot indicator under collection recipe boxes
let dots = [];

// Function to calculate total pages needed
function getTotalPages() {
    const boxesPerPage = getBoxesPerPage();
    return Math.ceil(collections.length / boxesPerPage);
}

// Function to create and manage dots based on screen size
function createDots() {
    const totalPages = getTotalPages();
    const dotsWrapper = document.querySelector('.dotContainer .dots');
    
    // Clear existing dots
    dotsWrapper.innerHTML = '';
    dots = [];
    
    // Create the right number of dots for current screen size
...
// Array for the featured section on homepage (expanded for responsive display)
let featured = [ 
    ...
    {id: 10, title: "Red Velvet Cupcakes", image_url: "./Images/cupcakes.jpg"},
];

// Function to determine number of featured recipes based on screen size
function getFeaturedItemsToShow() {
    ...
    if (computerQuery.matches) {
        return 4; // For large screens (PC)
    } else if (tabletQuery.matches) {
        return 3; // For medium screens (tablets)
    } else {
        return 3; // For small screens (mobile)
    }
}
...
// Function that shows the featured recipes in the recipe boxes (responsive)
function displayFeatured(featuredList){
    // Get number of items to show based on screen size
    const itemsToShow = getFeaturedItemsToShow();
    const recipesToShow = featuredList.slice(0, itemsToShow);
    
    let allFeatured = "";
    for(let i=0; i < recipesToShow.length; i++){
        let featured = `
            <div class="featuredBox" onclick="openRecipeDetailPage(${recipesToShow[i].id})">
                <img src="${recipesToShow[i].image_url}" alt="${recipesToShow[i].title}">
                <p> ${recipesToShow[i].title}</p>
            </div>
        `;
...
// Listen for screen size changes and re-render featured recipes
window.addEventListener("resize", () => {
    // Re-render when viewport size changes
    renderFeaturedRecipes();
});
...
function createSearchDropdown(inputElement, suggestions) {
    ...
    suggestions.forEach(suggestion => {
        ...
        item.addEventListener('click', () => {
            ...
            window.location.href = `./HTML/recipes.html?search=${encodeURIComponent(selectedSuggestion)}`;
        });
